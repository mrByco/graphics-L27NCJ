...
int distToPadY(Pad pad, Ball ball){
    return abs((pad.y + pad.height / 2) - ball.y);
}
...
void bounce_ball(Pong* pong)
{
    ...
    if (pong->ball.x - pong->ball.radius < 50) {
        ...
        pong->ball.speed_x *= -1.12;
        pong->ball.radius *= 0.95; 
        int padDist = distToPadY(pong->left_pad, pong->ball);
        if (padDist > pong->left_pad.height/2){
            stopBall(pong);
    ...
    if (pong->ball.x + pong->ball.radius > pong->width - 50) {
        ...
        int padDist = distToPadY(pong->right_pad, pong->ball);
        if (padDist > pong->height/2){
            stopBall(pong);
        }
    }
